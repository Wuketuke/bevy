...
use bevy_macro_utils::{derive_label, ensure_no_collision, BevyManifest};
...
#[proc_macro_derive(BundleFromComponent, attributes(bundle))]
/// Implement the `BundleFromComponent` trait for a Bundle.
pub fn derive_bundle_from_component(input: TokenStream) -> TokenStream {
    bundle::derive_bundle_from_component(input)
}
