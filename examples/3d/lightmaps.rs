...
const LIGHTMAP_BOX_LARGE_PATH: &str = "lightmaps/CornellBox-Large.zstd.ktx2";
const LIGHTMAP_BOX_SMALL_PATH: &str = "lightmaps/CornellBox-Small.zstd.ktx2";
const LIGHTMAP_BACKGROUND_PATH: &str = "lightmaps/CornellBox-Box.zstd.ktx2";
    ...
    for (entity, name, material) in meshes.iter() {
        if &**name == "large_box" {
            ...
            commands.entity(entity).insert(Lightmap {
                image: asset_server.load(LIGHTMAP_BOX_LARGE_PATH),
                ..default()
            });
            continue;
        }
        ...
        if &**name == "small_box" {
            materials.get_mut(material).unwrap().lightmap_exposure = exposure;
            commands.entity(entity).insert(Lightmap {
                image: asset_server.load(LIGHTMAP_BOX_SMALL_PATH),
                ..default()
            });
            continue;
        }
        ...
        if name.starts_with("cornell_box") {
            materials.get_mut(material).unwrap().lightmap_exposure = exposure;
            commands.entity(entity).insert(Lightmap {
                image: asset_server.load(LIGHTMAP_BACKGROUND_PATH),
                ..default()
            });
            continue;
        }
